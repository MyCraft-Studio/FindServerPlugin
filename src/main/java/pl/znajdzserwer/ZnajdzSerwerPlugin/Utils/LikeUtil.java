...
public class LikeUtil {
    ...
    public static void checkLike(Player player) {
        ...
        try {
            URL url = new URL("https://findserver.eu/application/api/addLike.php?ip="+player.getAddress().getAddress().getHostAddress()+"&address="+ ConfigSettings.getAddress());
            ...
            if(status < 200) {
                ...
                GlobalUtil.setLogs("An error has occurred "+status+", see docs on github");
                return;
            }
            if(status == 200) {
                ...
                GlobalUtil.setLogs(player.getName()+" gave like to the server and received a reward!");
